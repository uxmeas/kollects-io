      ...
      <table className="w-full">
              ...
              <tr key={moment.id} className={`border-b border-green-900 hover:bg-green-950 ${index % 2 === 0 ? 'bg-gray-950' : ''}`}>
                ...
                <td className="p-2 text-right">
                  {moment.purchasePrice !== undefined && moment.purchasePrice !== null ? (
                    moment.purchasePrice === 0 ? (
                      <span className="text-cyan-400">GIFT</span>
                    ) : (
                      `$${moment.purchasePrice.toLocaleString()}`
                    )
                  ) : '-'}
                </td>
        ...
        <tfoot>
          <tr className="border-t-2 border-green-600 font-bold">
            ...
            <td className="p-2 text-right">
              ${moments.filter(m => m.purchasePrice > 0).reduce((sum, m) => sum + (m.purchasePrice || 0), 0).toLocaleString()}
              <span className="text-xs text-gray-400 ml-2">
                ({moments.filter(m => m.purchasePrice === 0).length} gifts)
              </span>
            </td>
